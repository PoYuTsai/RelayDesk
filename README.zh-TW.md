...
`rc` 只是範例命名慣例，可以理解成 remote control 或 RelayDesk-controlled，不是 tmux 的必要功能。

使用固定 tmux session 名稱的原因是：只要你的電腦上 tmux server 還開著，你就可以之後從另一個 terminal、RelayDesk、或遠端控制流程重新 attach 到同一個 Claude Code / Codex CLI session。
...
- Claude Code：`/help`、`/clear`、`/compact`、`/resume`、`/remote-control`、`/diff`、`/code-review`。
- Codex CLI：`/clear`、`/new`、`/resume`、`/compact`、`/diff`、`/plan`、`/goal`、`/fork`。

`/round`、`/handoff` 這類指令不是 RelayDesk 或官方 provider 內建；如果你自己用 custom commands、skills、prompts、或專案設定定義它們，RelayDesk 仍然可以把它們送進對應 CLI。
...
這些按鈕只負責整理文字；你仍然可以自己決定要送到哪一個 runner。
