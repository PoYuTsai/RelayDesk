...
<p align="center">
  ...
  <a href="https://github.com/PoYuTsai/RelayDesk#english">English</a>
  ·
  <a href="https://github.com/PoYuTsai/RelayDesk#traditional-chinese"><strong>繁體中文</strong></a>
</p>
...
> GitHub repo 首頁預設渲染 `README.md`。上方語言連結會回到 repo 首頁的同頁語言區段；這份檔案是同步維護的獨立繁中翻譯版本。
...
1. 輸入任意專案名稱，例如 `My App`。
2. 輸入本機專案路徑，例如 `C:\Users\you\Desktop\MyApp`。
3. RelayDesk 會寫入 `relay.local.json`，並用 project id 自動產生預設 tmux session，例如 `rc-my-app` 和 `rc-codex-my-app`。
4. 再加入 Claude Code、Codex CLI 或 custom tmux runner。預設 Claude Code runner 會跑 `claude --model opus --effort xhigh`；如果你本來就有專案專用 shell script，可以把那段 script call 貼到 runner start command。
